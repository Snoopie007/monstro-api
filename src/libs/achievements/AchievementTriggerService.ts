...
export async function evaluateTriggers({
    ...
    try {
        const triggerIdToFind = triggerMap[triggerType].toString()

        const triggers = await db.query.triggeredAchievements.findMany({
            where: eq(triggeredAchievements.triggerId, triggerIdToFind),
            with: {
                achievement: true,
            },
        })
...
async function updateMemberAchievement(
    memberId: string,
...
async function awardPoints(
    ...
    await db
        ...
        .set({
            points: sql`${memberLocations.points} + ${points}`,
            updated: new Date(),
        })
        .where(
            and(
...
export async function evaluateAllTriggersForMember(
    ...
    const triggerTypes: TriggerEvaluation['triggerType'][] = [
        'attendances_count',
        'referrals_count',
        'plan_signup',
    ]
