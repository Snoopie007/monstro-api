...
import { authWithContext } from "../auth/server";
 ...
 * Generates a fresh JWT on each call using current session
 */
export async function getAuthenticatedSupabaseClient() {
  ...
  // Generate fresh Supabase JWT on-demand
  ...
  const supabase = createClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      global: {
        headers: {
          Authorization: `Bearer ${sbToken}`, // Fresh jwt from supabase
        },
      },
      realtime: {
				params: {
