...
import { memberContracts, memberLocations, memberPackages, memberSubscriptions } from "@/db/schemas";
...
export const mlDocsRoutes = new Elysia({ prefix: '/:lid/docs' })
    ...
    .post('/', async ({ memberId, params, status, body }: Props & { body: PostBody }) => {
        ...
        try {
            ...
            await s3.uploadBuffer(pdfBuffer, `members/${mid}/locations/${lid}/docs/${filename}`, "application/pdf");

            const [c] = await db.insert(memberContracts)
                .values({
                    signature,
                    templateId,
