...
type ScheduleOverdueCheckBody = {
    invoiceId: string;
    sendAt: string; // ISO timestamp
}

type CancelOverdueCheckParams = {
    invoiceId: string;
}

export async function overdueInvoiceRoutes(app: Elysia) {
    return app
        // Schedule the initial overdue check (30 mins after due date)
        .post('/overdue', async ({ body }) => {
            const { invoiceId, sendAt } = body as ScheduleOverdueCheckBody;
        ...
        // Cancel overdue check or reminder sequence
        .delete('/overdue/:invoiceId', async ({ params }) => {
            const { invoiceId } = params as CancelOverdueCheckParams;
