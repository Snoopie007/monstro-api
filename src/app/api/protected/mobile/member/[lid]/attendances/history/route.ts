...
export async function GET(req: NextRequest, props: { params: Promise<{ lid: number }> }) {
  ...
  try {
    ...
    // Get all reservations for member (without status filter)
    const reservations = await db.query.reservations.findMany({
      ...
      with: {
        ...
        attendance: true, // Changed from attendances (plural) to attendance (singular)
      }
    });
    ...
    reservations.forEach(reservation => {
      if (!reservation.attendance) return; // Changed to singular attendance
