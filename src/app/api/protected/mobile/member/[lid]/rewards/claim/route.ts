...
export async function POST(req: NextRequest, props: { params: Promise<{ lid: string }> }) {
	...
	try {
		...
		if (!(reward.claims.length < reward.limitPerMember && ml.points >= reward.requiredPoints))
		...
		await db.transaction(async (trx) => {
			...
			await trx.update(memberLocations).set({
				points: ml.points - reward.requiredPoints
			}).where(and(eq(memberLocations.memberId, memberId), eq(memberLocations.locationId, params.lid)));
