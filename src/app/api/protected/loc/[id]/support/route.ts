...
export async function POST(
    ...
    try {
        ...
        // If an assistant already exists for this location, return it
        ...
        if (existingAssistant) {
            return NextResponse.json({
                supportAssistant: existingAssistant,
                message: 'Support assistant already exists',
            })
        }

        // Create a default assistant
        const [created] = await db
            .insert(supportAssistants)
            .values({
                locationId: params.id,
                prompt: 'You are a helpful customer support assistant. You have access to member information tools to help with subscriptions, billing, and bookable sessions.',
                initialMessage:
        ...
        return NextResponse.json({
            supportAssistant: created,
            message: 'Support assistant created',
        })
    } catch (error) {
...
export async function PUT(
    ...
    try {
        ...
        if (knowledgeBase !== undefined)
            updateData.knowledgeBase = knowledgeBase
