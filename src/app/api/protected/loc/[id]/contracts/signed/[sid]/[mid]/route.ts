...
export async function GET(
	req: NextRequest,
	props: { params: Promise<{ id: number, sid: number, mid: number }> }
) {
	...
	try {
		...
		const contractId = Number(params.sid);
		...
		const [contract] = await db.select()
			...
			.where(
				and(
					eq(memberContracts.id, contractId),
					eq(memberContracts.memberId, Number(params.mid))
				)
			)
			.limit(1);
		...
		const htmlContent = generateContractHtml({
			...
			date: contract.signed ? new Date().toLocaleDateString('en-US', {
				year: 'numeric',
				month: 'long',
				day: 'numeric'
			}) : 'Unsigned',
		...
		response.headers.set('Content-Disposition', `attachment; filename="contract_${contract.id}.pdf"`);
		...
		return NextResponse.json({
			error: err instanceof Error ? err.message : 'Failed to download contract',
