// CONTACT INFO UPDATE START: New API route for member-specific company updates
...
export async function POST(
  ...
  try {
    ...
    // Update memberLocations with contact info
...
export async function GET(
  ...
  try {
    ...
    // Fetch memberLocation with contact info
    const memberLocation = await db.query.memberLocations.findFirst({
      where: (memberLocation, { eq, and }) =>
        and(
          eq(memberLocation.locationId, params.id),
      ...
      with: {
        location: true, // Include base location data for fallback
      },
    });
