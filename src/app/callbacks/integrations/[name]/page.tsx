...
async function completeIntegration(
  name: string,
  searchParams: IntegrationSearchParams
): Promise<boolean> {
  try {
    const location = await db.query.locations.findFirst({
      where: (loc, { eq }) => eq(loc.id, searchParams.state),
    });

    if (!location?.id) return false;

    let newIntegration: Partial<
      Omit<Integration, "locationId" | "service">
    > | null = null;

    if (name === "stripe") {
      if (!searchParams.scope) return false;
      const stripe = new MemberStripePayments();
      const token = await stripe.connectOAuth(
        searchParams.code,
        searchParams.scope
      );
      ...
      const tokenData = await exchangeCodeForToken(
        ...
        searchParams.realmId
      );
      if (!tokenData.access_token) return false;
      newIntegration = getQuickbooksSettings({
        ...tokenData,
      ...
      newIntegration = {
        ...
        accountId: tokenData.locationId || "",
...
function IntegrationResult({
  ...
  return (
    <div className="h-[100svh] bg-white dark:bg-white text-black dark:text-black w-full">
        ...
        <div className="text-center border px-6 py-10 rounded-sm border-gray-200 shadow-xs m-auto">
          {success ? (
            <div className="text-center flex flex-col items-center justify-center">
              ...
              <h1 className="text-2xl mb-4 font-poppins capitalize font-bold">
                Your <span className="capitalize">{integrationName}</span>{" "}
                account has been successfully connected.
              </h1>
              <p className="text-black/80">
                You'll be redirected back to Monstro.
              </p>
            </div>
          ) : (
            <div className="text-center flex flex-col items-center justify-center">
              ...
              <p>
                <span className="capitalize">{integrationName}</span> account
                could not be connected at this moment.
              </p>
            </div>
          )}
        </div>
      ...
      <meta
        httpEquiv="refresh"
        content={`3;url=/dashboard/location/${state}/settings/integrations`}
      />
    </div>
  );
}
