...
const useContentMenuActions = (editor: Editor, currentNode: Node | null, currentNodePos: number) => {
    ...
    const handleAdd = useCallback((action: CommandAction) => {
        if (currentNodePos !== -1) {
            ...
            editor
                .chain()
                .insertContentAt(insertPos, action)
                .focus(focusPos)
                .run()
        }
...
export default useContentMenuActions