...
export function SelectContract({ lid, onChange }: SelectContractProps) {
    ...
    return (
        <Select onValueChange={(e) => onChange(parseInt(e))} onOpenChange={(open) => open && getContracts()}>
            <FormControl>
                <SelectTrigger>
            ...
            <SelectContent>
                {contractsLoading ? (
                    ...
                    <div className='text-xs text-muted-foreground font-medium py-2 px-4'>No contracts found</div>
                )}
            </SelectContent>
        </Select>
