...
export function DayList({
  ...
  useEffect(() => {
    if (events) {
      const classes = events.filter((event) =>
  ...
  const handleRemoveReservation = async (
    ...
    try {
      ...
      if (isRecurring) {
        ...
        url = `/api/protected/loc/${lid}/members/${memberId}/reservations/${
        ...
        prev.map((evt) => {
          if (evt.id === event.id) {
            return {
              ...
              data: {
                ...evt.data,
                members: evt.data.members.filter(
  ...
  return (
    <div className="bg-background rounded-lg border border-foreground/10 flex-1 h-full">
      <div className="flex flex-col flex-1 h-full p-4 space-y-2">
        <div className="text-sm font-semibold ">Session for {today}</div>
