...
export function useSupportRealtime({
		...
		(payload: any) => {
			try {
				...
				const formattedConversation: SupportConversation = {
					...
					takenOverAt: conversation.taken_over_at ? new Date(conversation.taken_over_at) : null,
				...
				if (conversation.location_id === locationId) {
					...
					if (
						...
						callbacksRef.current.onVendorActiveChange?.(conversation.id, true);
					}
				}
			} catch (error) {
	...
	}, [handleMessageInsert, handleConversationUpdate]);
	...
	const isAiMode = useCallback(
		(conv?: SupportConversation): boolean => {
			...
			return targetConversation ? !targetConversation.isVendorActive : false;
		},
		[conversation]
	);
