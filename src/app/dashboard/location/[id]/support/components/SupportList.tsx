...
export function SupportList({ lid }: { lid: string }) {
    ...
    const filteredConversations = useMemo(() => {
        ...
        return conversations.filter((c) => {
            if (search) {
                const name =
                    `${c.member?.firstName} ${c.member?.lastName}`.toLowerCase()
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                ...
                                >
                                    <Link
                                        href={`/dashboard/location/${lid}/support/settings`}
                                    >
                                        <Settings size={16} />
                                    </Link>
                                </Button>
                    ...
                    <div className="flex flex-row gap-2 items-center">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button
                                    variant="ghost"
                                    className=" h-auto rounded-full font-semibold  bg-foreground/5 px-3 py-1 text-xs text-center"
                                >
                                    {
                                        byStatus.find(
                                            (s) => s.status === status
                            ...
                            >
                                {byStatus.map((s) => (
                                    <DropdownMenuItem
                                        ...
                                        onClick={() =>
                                            setStatus(
                                                s.status as SupportConversationStatus
                                            )
                                        }
...
function ConversationItem({ conversation, isSelected }: ConversationItemProps) {
    ...
    return (
        <li
            ...
            className={cn(
                'px-4 py-3 hover:bg-foreground/5 cursor-pointer rounded-lg flex flex-row items-center gap-2 ',
                {
                ...
                <div className="space-y-0 w-full">
                    ...
                    <div className="text-[0.7rem] text-muted-foreground flex flex-row gap-2 justify-between items-center">
                        ...
                        <span className=" text-muted-foreground">
                            {formatDistance(conversation.created, new Date(), {
                                addSuffix: true,
                            })}
                        </span>
                    </div>
