...
function BadgeItem({ badge, vendorBadges }: { badge: AchivementBadge, vendorBadges: VendorBadge[] }) {
    const vendorBadge = vendorBadges?.find((item: VendorBadge) => item.badgeId === badge.id)

    return (
        <div className={cn('col-span-1 max-h-[260px] h-[260px] border border-foreground/10 rounded-xs p-6 text-left relative',
            ...
            <div className='flex flex-col gap-2 items-center relative'>
                ...
                <div className='relative flex-initial'>
                    <Image src={`/images/icons/badges/${badge.icon}`} alt={badge.name} width={100} height={100}
                        className={cn('grayscale -z-10', vendorBadge?.completed ? 'grayscale-0' : ' grayscale')} />
                    <div className='absolute bottom-0 right-0 w-full h-full z-20 flex flex-col items-center justify-center'>
                        <div className='mt-[40px] w-full flex flex-col items-center justify-center'>
                            <Progress
                                value={(vendorBadge ? vendorBadge.progress : 0) / (badge.requiredCount || 1) * 100}
                            ...
                            <p className="text-xs font-medium mt-1 bg-black/50 rounded-xs px-2 inline-flex items-center justify-center">
                                {vendorBadge ? vendorBadge.progress : 0}/{badge.requiredCount}
                            </p>
                        </div>
                    </div>
                </div>
                <div className='flex flex-col gap-1 flex-1 text-center  '>
                    <span className={cn('text-sm font-medium', vendorBadge?.completed ? 'text-green-500' : '')}>
                        {badge.name}
                    </span>
                    <span className={cn('text-xs text-muted-foreground', vendorBadge?.completed ? 'text-green-500/50' : '')}>
                        {badge.description}
                    </span>
