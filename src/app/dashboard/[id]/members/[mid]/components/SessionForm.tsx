...
export function SessionForm({ params, progress }: SessionFormProps) {
    ...
    useEffect(() => {
        if (selectedDay) {
            if (selectedDay === 8) {
                setFilteredSessions(originalSessions);
            } else {
                const day = groupedSessions.get(selectedDay);
                setFilteredSessions(day || []);
            }
        }
    ...
    async function fetchSessions() {
        const { result, error } = await tryCatch(
            fetch(`/api/protected/${params.id}/programs/14/levels/1/sessions`)
        );
    ...
    const getTotalSessionsText = useMemo(() => {
        ...
        return `${sessionIds.length}/${progress.plan.classLimitThreshold}`;
    }, [sessionIds.length]);
            ...
            <DialogBody >
                <div className="space-y-4 w-full">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="inline-flex flex-row bg-background rounded-sm border border-foreground/10 overflow-hidden">
                            {DAYS.map((day, i) => (
                                <div key={i} className={cn("text-xs hover:bg-indigo-500 cursor-pointer font-medium ",
                                    "px-2 border-r border-foreground/10 last:border-r-0 py-1.5 flex items-center justify-center",
...
function SessionItem({ session, isFull, handleSessionSelection, sessionIds, progress }: SessionItemProps) {
    return (
        ...
        >
            <div className="text-xs text-foreground/50">{DAYS.find(day => day.value === session.day)?.label}</div>
