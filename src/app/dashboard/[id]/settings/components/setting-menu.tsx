...
export default function SettingMenu({ locationId }: { locationId: string }) {
    return (
        <aside className="col-span-2">
            <ul className="text-foreground flex flex-col gap-1">
                {SettingMenuItems.map((item, i) => (
                    <li key={i} className={cn("font-semibold ")}>
                        <Link href={`/dashboard/${locationId}/settings/${item.path}`}
                            className='block h-full w-full  px-4 py-2 hover:bg-accent rounded-sm'>
                            {item.name}
                        </Link>
                    </li>
                ))}
